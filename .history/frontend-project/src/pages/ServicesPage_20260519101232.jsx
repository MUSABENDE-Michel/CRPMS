...
const ServicesPage = () => {
  ...
  const checkDuplicateCode = (code, excludeId = null) => {
    ...
    const existingService = services.find(service =>
      service.serviceCode === formattedCode && service._id !== excludeId
    );

    if (existingService) {
      setErrors(prev => ({ ...prev, serviceCode: 'This service code already exists!' }));
      return true;
    }
    return false;
  };
  ...
  const checkDuplicateName = (name, excludeId = null) => {
    ...
    const existingService = services.find(service =>
      service.serviceName.toLowerCase() === name.toLowerCase() && service._id !== excludeId
    );

    if (existingService) {
      setErrors(prev => ({ ...prev, serviceName: 'This service name already exists!' }));
      return true;
    }
    return false;
  };
  ...
  const validateForm = () => {
    ...
    if (!validateRequired(formData.serviceCode)) {
      ...
      const isDuplicate = checkDuplicateCode(formData.serviceCode, editingId);
      if (isDuplicate) {
        newErrors.serviceCode = errors.serviceCode || 'This service code already exists!';
      }
    }
    ...
    if (!validateRequired(formData.serviceName)) {
      ...
      const isDuplicateName = checkDuplicateName(formData.serviceName, editingId);
      if (isDuplicateName) {
        newErrors.serviceName = errors.serviceName || 'This service name already exists!';
      }
    }
    ...
    try {
      ...
      if (errorMessage.includes('duplicate') || errorMessage.includes('already exists')) {
        if (errorMessage.includes('code')) {
          setErrors(prev => ({ ...prev, serviceCode: errorMessage }));
        ...
        <div className="space-y-4">
          <FormGroup label="Service Code" error={errors.serviceCode} required>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              ⚠️ Service code must be unique. Use uppercase letters, numbers, and hyphens only
            </p>
          </FormGroup>
