...
const CarsPage = () => {
  ...
  const checkDuplicatePlate = async (plateNumber, excludeId = null) => {
    if (!plateNumber) return false;

    const formattedPlate = plateNumber.toUpperCase().replace(/\s/g, '');
    const existingCar = cars.find(car =>
      car.plateNumber === formattedPlate && car._id !== excludeId
    );

    if (existingCar) {
      setErrors(prev => ({ ...prev, plateNumber: 'This plate number already exists!' }));
      return true;
    }
    return false;
  };
  ...
  const checkDuplicatePhone = async (phone, excludeId = null) => {
    ...
    const existingCar = cars.find(car =>
      car.driverPhone === cleanedPhone && car._id !== excludeId
    );

    if (existingCar) {
      setErrors(prev => ({ ...prev, driverPhone: 'This phone number is already registered to another car!' }));
      return true;
    }
    return false;
  };
  ...
  // Enhanced validation with Rwandan rules and duplicate checks
  const validateForm = async () => {
    ...
    if (!validateRequired(formData.plateNumber)) {
      ...
      const isDuplicate = await checkDuplicatePlate(formData.plateNumber, editingId);
      if (isDuplicate) {
        newErrors.plateNumber = errors.plateNumber || 'This plate number already exists!';
      }
    }
    ...
    if (!validateRequired(formData.driverPhone)) {
      ...
      const isDuplicatePhone = await checkDuplicatePhone(formData.driverPhone, editingId);
      if (isDuplicatePhone) {
        newErrors.driverPhone = errors.driverPhone || 'This phone number is already registered to another car!';
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 'Error saving car';
      addToast(errorMessage, 'error');

      // Handle duplicate error from backend
      if (errorMessage.includes('duplicate') || errorMessage.includes('already exists')) {
        if (errorMessage.includes('plate')) {
          setErrors(prev => ({ ...prev, plateNumber: errorMessage }));
        } else if (errorMessage.includes('phone')) {
          setErrors(prev => ({ ...prev, driverPhone: errorMessage }));
        }
      }
    } finally {
      setLoading(false);
        ...
        <div className="space-y-4">
          <FormGroup label="Plate Number" error={errors.plateNumber} required>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              ⚠️ Plate number must be unique. Format: RAB123C, ABC123D (Rwanda standard)
            </p>
          </FormGroup>
          ...
          <FormGroup label="Driver Phone" error={errors.driverPhone} required>
            ...
            />
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              ⚠️ Phone number must be unique per car. Rwandan format: 078, 079, or 072 followed by 7 digits
            </p>
          </FormGroup>
