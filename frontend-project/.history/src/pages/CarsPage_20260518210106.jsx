...
const CarsPage = () => {
  ...
  const validateForm = () => {
    ...
    if (!validateRequired(formData.plateNumber)) {
      ...
      newErrors.plateNumber = 'Invalid plate number format. Examples: RAB123C, ABC123D, RAB 123C';
    }
    ...
    if (!validateRequired(formData.driverPhone)) {
      ...
      newErrors.driverPhone = 'Invalid Rwandan phone number. Format: 0788XXXXXX or 0728XXXXXX (10 digits)';
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-4 max-h-96 overflow-y-auto">
              <FormGroup label="Plate Number" error={errors.plateNumber} required>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  Format: RAB123C, ABC123D, or RAB 123C
                </p>
              </FormGroup>
              ...
              <FormGroup label="Driver Phone" error={errors.driverPhone} required>
                <Input
                  ...
                  onChange={handleChange}
                  placeholder="0788XXXXXX or 0728XXXXXX"
                  error={!!errors.driverPhone}
                />
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  Rwandan phone number: 10 digits starting with 07
                </p>
              </FormGroup>
      ...
      {viewHistory && selectedCar && (
        <div className="modal-overlay" onClick={() => setViewHistory(false)}>
          <div className="modal-content max-w-2xl" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">
              Service History - {formatPlateNumber(selectedCar.plateNumber)}
            </h2>
...
// Helper function for Rwandan phone formatting
...
export default CarsPage;import React, { useState, useEffect, useCallback } from 'react';
import { carAPI, serviceRecordAPI } from '../api/apiClient';
import { useToast } from '../hooks/useCustomHooks';
import { 
  formatCurrency, 
